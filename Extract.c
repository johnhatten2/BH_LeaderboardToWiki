...
const char* FILE_ORDER[6] = {TG_RANKS_PATH, EX_TG_RANKS_PATH, TG_POINTS_PATH, PVP_RANKS_PATH, EX_PVP_RANKS_PATH, PVP_POINTS_PATH};
...
void ExtractData(t_Leaderboard* Leaderboard, char* FilePath, int week, int PointsOrRanks, int MainOrEx)
{
    ...
    while(fgets(line, LINE_SIZE, fp))
    {
        ...
        for(int i = atoi(ZoneMin); i < atoi(ZoneMax) + 1; i++)
        {
            ...
            if(MainOrEx == MAIN)
                AddLeaderboardData(Leaderboard, RewardLink, Rank, Reward, atoi(Qty), i, week);
...
void GenerateLeaderboardPage(t_Leaderboard Leaderboard[])
{
    ...
    t_RewardLinks* LinksArr[6];
    LinksArr[0] = &(Leaderboard[TG_RANKS].RewardLinks[MAIN]);
    LinksArr[1] = &(Leaderboard[TG_RANKS].RewardLinks[EX]);
    LinksArr[2] = &(Leaderboard[TG_POINTS].RewardLinks[MAIN]);
    LinksArr[3] = &(Leaderboard[PVP_RANKS].RewardLinks[MAIN]);
    LinksArr[4] = &(Leaderboard[PVP_RANKS].RewardLinks[EX]);
    LinksArr[5] = &(Leaderboard[PVP_POINTS].RewardLinks[MAIN]);
    ...
    for(int i = 0; i < 6; i++)
    {
        ...
        //for each Links in a file
        for(int j = 0; j < LinksArr[i]->NbLink; j++)
            fprintf(fp, "   %s\n", LinksArr[i]->Links[j]);
    }
